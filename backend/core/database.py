...
class DatabaseManager:
    ...
    def _sanitize_query_params(url):
        ...
        asyncpg uses a native ``ssl`` parameter instead of ``sslmode``.
        """
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Check if we're in a Lambda environment using the template's existing flag.
            is_lambda = os.environ.get("IS_LAMBDA", "").lower() in ("true", "1", "yes")

            if is_lambda:
                if os.environ.get("DB_DISABLE_POOL", "").lower() in ("true", "1", "yes"):
                    ...
                    engine_kwargs["pool_size"] = int(os.environ.get("DB_POOL_SIZE", "1"))
                    engine_kwargs["max_overflow"] = int(os.environ.get("DB_MAX_OVERFLOW", "2"))
                    engine_kwargs["pool_recycle"] = int(os.environ.get("DB_POOL_RECYCLE", "280"))
                    logger.info("Using pooled connections for Lambda environment (reuse across warm invocations)")
            else:
            ...
            # Handle sslmode for asyncpg: asyncpg does not accept 'sslmode' as a
            # query parameter. If the original DATABASE_URL contained sslmode=require
            # (or similar), we already stripped it in _sanitize_query_params but need
            # to enable SSL natively via connect_args.
            if "asyncpg" in database_url:
                original_url = settings.database_url or ""
                sslmode_match = re.search(r"[?&]sslmode=([^&]+)", original_url)
                if sslmode_match:
                    sslmode_value = sslmode_match.group(1).lower()
                    if sslmode_value in ("require", "verify-ca", "verify-full", "prefer"):
                        import ssl as _ssl
                        ssl_ctx = _ssl.create_default_context()
                        ssl_ctx.check_hostname = False
                        ssl_ctx.verify_mode = _ssl.CERT_NONE
                        engine_kwargs.setdefault("connect_args", {})["ssl"] = ssl_ctx
                        logger.info(f"Enabled asyncpg SSL (converted from sslmode={sslmode_value})")
    ...
    async def create_tables(self):
        ...
        if "MGX_IGNORE_INIT_TABLES" in os.environ:
            logger.info("Ignore tables initialization")
            return
