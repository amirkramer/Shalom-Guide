...
# ------------------ Service Layer ------------------
class Rental_companiesService:
    """Service layer for Rental_companies operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Rental_companies]:
        """Create a new rental_companies"""
        try:
            obj = Rental_companies(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created rental_companies with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating rental_companies: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Rental_companies]:
        """Get rental_companies by ID"""
        try:
            query = select(Rental_companies).where(Rental_companies.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rental_companies {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of rental_companiess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Rental_companies, field):
                        column = getattr(Rental_companies, field)
            ...
            stmt = select(Rental_companies, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Rental_companies, field_name):
                        stmt = stmt.order_by(getattr(Rental_companies, field_name).desc())
                else:
                    if hasattr(Rental_companies, sort):
                        stmt = stmt.order_by(getattr(Rental_companies, sort))
            else:
                stmt = stmt.order_by(Rental_companies.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Rental_companies)
                for condition in conditions:
            ...
            logger.error(f"Error fetching rental_companies list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Rental_companies]:
        """Update rental_companies"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rental_companies {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated rental_companies {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating rental_companies {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete rental_companies"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Rental_companies {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted rental_companies {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting rental_companies {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Rental_companies]:
        """Get rental_companies by any field"""
        try:
            if not hasattr(Rental_companies, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rental_companies")
            column = getattr(Rental_companies, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Rental_companies).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching rental_companies by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Rental_companies]:
        """Get list of rental_companiess filtered by field"""
        try:
            if not hasattr(Rental_companies, field_name):
                raise ValueError(f"Field {field_name} does not exist on Rental_companies")
            column = getattr(Rental_companies, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Rental_companies.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching rental_companiess by {field_name}: {str(e)}")
            raise