...
# ------------------ Service Layer ------------------
class Tourist_sitesService:
    """Service layer for Tourist_sites operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Tourist_sites]:
        """Create a new tourist_sites"""
        try:
            obj = Tourist_sites(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created tourist_sites with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating tourist_sites: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Tourist_sites]:
        """Get tourist_sites by ID"""
        try:
            query = select(Tourist_sites).where(Tourist_sites.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tourist_sites {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of tourist_sitess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Tourist_sites, field):
                        column = getattr(Tourist_sites, field)
            ...
            stmt = select(Tourist_sites, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Tourist_sites, field_name):
                        stmt = stmt.order_by(getattr(Tourist_sites, field_name).desc())
                else:
                    if hasattr(Tourist_sites, sort):
                        stmt = stmt.order_by(getattr(Tourist_sites, sort))
            else:
                stmt = stmt.order_by(Tourist_sites.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Tourist_sites)
                for condition in conditions:
            ...
            logger.error(f"Error fetching tourist_sites list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Tourist_sites]:
        """Update tourist_sites"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Tourist_sites {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated tourist_sites {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating tourist_sites {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete tourist_sites"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Tourist_sites {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted tourist_sites {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting tourist_sites {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Tourist_sites]:
        """Get tourist_sites by any field"""
        try:
            if not hasattr(Tourist_sites, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tourist_sites")
            column = getattr(Tourist_sites, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Tourist_sites).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tourist_sites by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Tourist_sites]:
        """Get list of tourist_sitess filtered by field"""
        try:
            if not hasattr(Tourist_sites, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tourist_sites")
            column = getattr(Tourist_sites, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Tourist_sites)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Tourist_sites.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching tourist_sitess by {field_name}: {str(e)}")
            raise