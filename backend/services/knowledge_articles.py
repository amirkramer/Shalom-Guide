...
class Knowledge_articlesService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Knowledge_articles, field):
                        column = getattr(Knowledge_articles, field)
            ...
            stmt = select(Knowledge_articles, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Knowledge_articles, field_name):
                        stmt = stmt.order_by(getattr(Knowledge_articles, field_name).desc())
                else:
                    if hasattr(Knowledge_articles, sort):
                        stmt = stmt.order_by(getattr(Knowledge_articles, sort))
            else:
                stmt = stmt.order_by(Knowledge_articles.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Knowledge_articles)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Knowledge_articles]:
        ...
        try:
            ...
            column = getattr(Knowledge_articles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Knowledge_articles).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
