...
from models.dashboard_modules import Dashboard_modules

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Dashboard_modulesService:
    """Service layer for Dashboard_modules operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Dashboard_modules]:
        """Create a new dashboard_modules"""
        try:
            obj = Dashboard_modules(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created dashboard_modules with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating dashboard_modules: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Dashboard_modules]:
        """Get dashboard_modules by ID"""
        try:
            query = select(Dashboard_modules).where(Dashboard_modules.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching dashboard_modules {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of dashboard_moduless"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Dashboard_modules, field):
                        column = getattr(Dashboard_modules, field)
            ...
            stmt = select(Dashboard_modules, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Dashboard_modules, field_name):
                        stmt = stmt.order_by(getattr(Dashboard_modules, field_name).desc())
                else:
                    if hasattr(Dashboard_modules, sort):
                        stmt = stmt.order_by(getattr(Dashboard_modules, sort))
            else:
                stmt = stmt.order_by(Dashboard_modules.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Dashboard_modules)
                for condition in conditions:
            ...
            logger.error(f"Error fetching dashboard_modules list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Dashboard_modules]:
        """Update dashboard_modules"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Dashboard_modules {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated dashboard_modules {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating dashboard_modules {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete dashboard_modules"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Dashboard_modules {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted dashboard_modules {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting dashboard_modules {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Dashboard_modules]:
        """Get dashboard_modules by any field"""
        try:
            if not hasattr(Dashboard_modules, field_name):
                raise ValueError(f"Field {field_name} does not exist on Dashboard_modules")
            column = getattr(Dashboard_modules, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Dashboard_modules).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching dashboard_modules by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Dashboard_modules]:
        """Get list of dashboard_moduless filtered by field"""
        try:
            if not hasattr(Dashboard_modules, field_name):
                raise ValueError(f"Field {field_name} does not exist on Dashboard_modules")
            column = getattr(Dashboard_modules, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Dashboard_modules.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching dashboard_moduless by {field_name}: {str(e)}")
            raise