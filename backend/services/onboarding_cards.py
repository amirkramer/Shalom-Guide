...
from models.onboarding_cards import Onboarding_cards

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Onboarding_cardsService:
    """Service layer for Onboarding_cards operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Onboarding_cards]:
        """Create a new onboarding_cards"""
        try:
            obj = Onboarding_cards(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created onboarding_cards with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating onboarding_cards: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Onboarding_cards]:
        """Get onboarding_cards by ID"""
        try:
            query = select(Onboarding_cards).where(Onboarding_cards.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching onboarding_cards {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of onboarding_cardss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Onboarding_cards, field):
                        column = getattr(Onboarding_cards, field)
            ...
            stmt = select(Onboarding_cards, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Onboarding_cards, field_name):
                        stmt = stmt.order_by(getattr(Onboarding_cards, field_name).desc())
                else:
                    if hasattr(Onboarding_cards, sort):
                        stmt = stmt.order_by(getattr(Onboarding_cards, sort))
            else:
                stmt = stmt.order_by(Onboarding_cards.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Onboarding_cards)
                for condition in conditions:
            ...
            logger.error(f"Error fetching onboarding_cards list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Onboarding_cards]:
        """Update onboarding_cards"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Onboarding_cards {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated onboarding_cards {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating onboarding_cards {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete onboarding_cards"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Onboarding_cards {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted onboarding_cards {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting onboarding_cards {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Onboarding_cards]:
        """Get onboarding_cards by any field"""
        try:
            if not hasattr(Onboarding_cards, field_name):
                raise ValueError(f"Field {field_name} does not exist on Onboarding_cards")
            column = getattr(Onboarding_cards, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Onboarding_cards).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching onboarding_cards by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Onboarding_cards]:
        """Get list of onboarding_cardss filtered by field"""
        try:
            if not hasattr(Onboarding_cards, field_name):
                raise ValueError(f"Field {field_name} does not exist on Onboarding_cards")
            column = getattr(Onboarding_cards, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Onboarding_cards)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Onboarding_cards.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching onboarding_cardss by {field_name}: {str(e)}")
            raise