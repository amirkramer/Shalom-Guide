...
from models.shabbat_times import Shabbat_times

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Shabbat_timesService:
    """Service layer for Shabbat_times operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Shabbat_times]:
        """Create a new shabbat_times"""
        try:
            obj = Shabbat_times(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created shabbat_times with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating shabbat_times: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Shabbat_times]:
        """Get shabbat_times by ID"""
        try:
            query = select(Shabbat_times).where(Shabbat_times.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shabbat_times {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of shabbat_timess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Shabbat_times, field):
                        column = getattr(Shabbat_times, field)
            ...
            stmt = select(Shabbat_times, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Shabbat_times, field_name):
                        stmt = stmt.order_by(getattr(Shabbat_times, field_name).desc())
                else:
                    if hasattr(Shabbat_times, sort):
                        stmt = stmt.order_by(getattr(Shabbat_times, sort))
            else:
                stmt = stmt.order_by(Shabbat_times.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Shabbat_times)
                for condition in conditions:
            ...
            logger.error(f"Error fetching shabbat_times list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Shabbat_times]:
        """Update shabbat_times"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shabbat_times {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated shabbat_times {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating shabbat_times {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete shabbat_times"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shabbat_times {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted shabbat_times {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting shabbat_times {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Shabbat_times]:
        """Get shabbat_times by any field"""
        try:
            if not hasattr(Shabbat_times, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shabbat_times")
            column = getattr(Shabbat_times, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shabbat_times).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shabbat_times by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Shabbat_times]:
        """Get list of shabbat_timess filtered by field"""
        try:
            if not hasattr(Shabbat_times, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shabbat_times")
            column = getattr(Shabbat_times, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shabbat_times)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Shabbat_times.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching shabbat_timess by {field_name}: {str(e)}")
            raise