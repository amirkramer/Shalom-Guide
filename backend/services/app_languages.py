...
from models.app_languages import App_languages

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class App_languagesService:
    """Service layer for App_languages operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[App_languages]:
        """Create a new app_languages"""
        try:
            obj = App_languages(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created app_languages with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating app_languages: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[App_languages]:
        """Get app_languages by ID"""
        try:
            query = select(App_languages).where(App_languages.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_languages {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of app_languagess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(App_languages, field):
                        column = getattr(App_languages, field)
            ...
            stmt = select(App_languages, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(App_languages, field_name):
                        stmt = stmt.order_by(getattr(App_languages, field_name).desc())
                else:
                    if hasattr(App_languages, sort):
                        stmt = stmt.order_by(getattr(App_languages, sort))
            else:
                stmt = stmt.order_by(App_languages.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(App_languages)
                for condition in conditions:
            ...
            logger.error(f"Error fetching app_languages list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[App_languages]:
        """Update app_languages"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"App_languages {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated app_languages {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating app_languages {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete app_languages"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"App_languages {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted app_languages {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting app_languages {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[App_languages]:
        """Get app_languages by any field"""
        try:
            if not hasattr(App_languages, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_languages")
            column = getattr(App_languages, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(App_languages).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_languages by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[App_languages]:
        """Get list of app_languagess filtered by field"""
        try:
            if not hasattr(App_languages, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_languages")
            column = getattr(App_languages, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(App_languages.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching app_languagess by {field_name}: {str(e)}")
            raise