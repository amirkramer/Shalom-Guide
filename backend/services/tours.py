...
# ------------------ Service Layer ------------------
class ToursService:
    """Service layer for Tours operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Tours]:
        """Create a new tours"""
        try:
            if user_id:
            ...
            logger.info(f"Created tours with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating tours: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for tours {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Tours]:
        """Get tours by ID (user can only see their own records)"""
        try:
            query = select(Tours).where(Tours.id == obj_id)
            if user_id:
                query = query.where(Tours.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tours {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of tourss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Tours.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Tours, field):
                        column = getattr(Tours, field)
            ...
            stmt = select(Tours, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Tours, field_name):
                        stmt = stmt.order_by(getattr(Tours, field_name).desc())
                else:
                    if hasattr(Tours, sort):
                        stmt = stmt.order_by(getattr(Tours, sort))
            else:
                stmt = stmt.order_by(Tours.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Tours)
                for condition in conditions:
            ...
            logger.error(f"Error fetching tours list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Tours]:
        """Update tours (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Tours {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated tours {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating tours {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete tours (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Tours {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted tours {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting tours {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Tours]:
        """Get tours by any field"""
        try:
            if not hasattr(Tours, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tours")
            column = getattr(Tours, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Tours).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching tours by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Tours]:
        """Get list of tourss filtered by field"""
        try:
            if not hasattr(Tours, field_name):
                raise ValueError(f"Field {field_name} does not exist on Tours")
            column = getattr(Tours, field_name)
            ...
            logger.error(f"Error fetching tourss by {field_name}: {str(e)}")
            raise