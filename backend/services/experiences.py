...
class ExperiencesService:
    """Service layer for Experiences operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Experiences]:
        ...
        try:
            ...
            logger.info(f"Created experiences with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating experiences: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Experiences]:
        """Get experiences by ID"""
        try:
            query = select(Experiences).where(Experiences.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching experiences {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of experiencess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Experiences, field):
                        column = getattr(Experiences, field)
            ...
            stmt = select(Experiences, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Experiences, field_name):
                        stmt = stmt.order_by(getattr(Experiences, field_name).desc())
                else:
                    if hasattr(Experiences, sort):
                        stmt = stmt.order_by(getattr(Experiences, sort))
            else:
                stmt = stmt.order_by(Experiences.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Experiences)
                for condition in conditions:
            ...
            logger.error(f"Error fetching experiences list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Experiences]:
        """Update experiences"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Experiences {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated experiences {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating experiences {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Experiences {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted experiences {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting experiences {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Experiences]:
        """Get experiences by any field"""
        try:
            if not hasattr(Experiences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Experiences")
            column = getattr(Experiences, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Experiences).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching experiences by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Experiences]:
        """Get list of experiencess filtered by field"""
        try:
            if not hasattr(Experiences, field_name):
                raise ValueError(f"Field {field_name} does not exist on Experiences")
            column = getattr(Experiences, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Experiences.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching experiencess by {field_name}: {str(e)}")
            raise