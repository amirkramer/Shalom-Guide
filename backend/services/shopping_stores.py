...
# ------------------ Service Layer ------------------
class Shopping_storesService:
    """Service layer for Shopping_stores operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Shopping_stores]:
        """Create a new shopping_stores"""
        try:
            obj = Shopping_stores(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created shopping_stores with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating shopping_stores: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Shopping_stores]:
        """Get shopping_stores by ID"""
        try:
            query = select(Shopping_stores).where(Shopping_stores.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shopping_stores {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of shopping_storess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Shopping_stores, field):
                        column = getattr(Shopping_stores, field)
            ...
            stmt = select(Shopping_stores, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Shopping_stores, field_name):
                        stmt = stmt.order_by(getattr(Shopping_stores, field_name).desc())
                else:
                    if hasattr(Shopping_stores, sort):
                        stmt = stmt.order_by(getattr(Shopping_stores, sort))
            else:
                stmt = stmt.order_by(Shopping_stores.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Shopping_stores)
                for condition in conditions:
            ...
            logger.error(f"Error fetching shopping_stores list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Shopping_stores]:
        """Update shopping_stores"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shopping_stores {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated shopping_stores {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating shopping_stores {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete shopping_stores"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shopping_stores {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted shopping_stores {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting shopping_stores {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Shopping_stores]:
        """Get shopping_stores by any field"""
        try:
            if not hasattr(Shopping_stores, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shopping_stores")
            column = getattr(Shopping_stores, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shopping_stores).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shopping_stores by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Shopping_stores]:
        """Get list of shopping_storess filtered by field"""
        try:
            if not hasattr(Shopping_stores, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shopping_stores")
            column = getattr(Shopping_stores, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shopping_stores)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Shopping_stores.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching shopping_storess by {field_name}: {str(e)}")
            raise