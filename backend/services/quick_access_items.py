...
from models.quick_access_items import Quick_access_items

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Quick_access_itemsService:
    """Service layer for Quick_access_items operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Quick_access_items]:
        """Create a new quick_access_items"""
        try:
            obj = Quick_access_items(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created quick_access_items with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating quick_access_items: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Quick_access_items]:
        """Get quick_access_items by ID"""
        try:
            query = select(Quick_access_items).where(Quick_access_items.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching quick_access_items {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of quick_access_itemss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Quick_access_items, field):
                        column = getattr(Quick_access_items, field)
            ...
            stmt = select(Quick_access_items, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Quick_access_items, field_name):
                        stmt = stmt.order_by(getattr(Quick_access_items, field_name).desc())
                else:
                    if hasattr(Quick_access_items, sort):
                        stmt = stmt.order_by(getattr(Quick_access_items, sort))
            else:
                stmt = stmt.order_by(Quick_access_items.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Quick_access_items)
                for condition in conditions:
            ...
            logger.error(f"Error fetching quick_access_items list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Quick_access_items]:
        """Update quick_access_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Quick_access_items {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated quick_access_items {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating quick_access_items {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete quick_access_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Quick_access_items {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted quick_access_items {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting quick_access_items {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Quick_access_items]:
        """Get quick_access_items by any field"""
        try:
            if not hasattr(Quick_access_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Quick_access_items")
            column = getattr(Quick_access_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Quick_access_items).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching quick_access_items by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Quick_access_items]:
        """Get list of quick_access_itemss filtered by field"""
        try:
            if not hasattr(Quick_access_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Quick_access_items")
            column = getattr(Quick_access_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Quick_access_items)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Quick_access_items.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching quick_access_itemss by {field_name}: {str(e)}")
            raise