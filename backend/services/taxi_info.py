...
# ------------------ Service Layer ------------------
class Taxi_infoService:
    """Service layer for Taxi_info operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Taxi_info]:
        ...
        try:
            ...
            logger.info(f"Created taxi_info with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating taxi_info: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Taxi_info]:
        """Get taxi_info by ID"""
        try:
            query = select(Taxi_info).where(Taxi_info.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching taxi_info {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of taxi_infos"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Taxi_info, field):
                        column = getattr(Taxi_info, field)
            ...
            stmt = select(Taxi_info, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Taxi_info, field_name):
                        stmt = stmt.order_by(getattr(Taxi_info, field_name).desc())
                else:
                    if hasattr(Taxi_info, sort):
                        stmt = stmt.order_by(getattr(Taxi_info, sort))
            else:
                stmt = stmt.order_by(Taxi_info.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Taxi_info)
                for condition in conditions:
            ...
            logger.error(f"Error fetching taxi_info list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Taxi_info]:
        """Update taxi_info"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Taxi_info {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated taxi_info {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating taxi_info {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Taxi_info {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted taxi_info {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting taxi_info {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Taxi_info]:
        """Get taxi_info by any field"""
        try:
            if not hasattr(Taxi_info, field_name):
                raise ValueError(f"Field {field_name} does not exist on Taxi_info")
            column = getattr(Taxi_info, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Taxi_info).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching taxi_info by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Taxi_info]:
        """Get list of taxi_infos filtered by field"""
        try:
            if not hasattr(Taxi_info, field_name):
                raise ValueError(f"Field {field_name} does not exist on Taxi_info")
            column = getattr(Taxi_info, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Taxi_info.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching taxi_infos by {field_name}: {str(e)}")
            raise