...
class RestaurantsService:
    """Service layer for Restaurants operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Restaurants]:
        ...
        try:
            ...
            logger.info(f"Created restaurants with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating restaurants: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Restaurants]:
        """Get restaurants by ID"""
        try:
            query = select(Restaurants).where(Restaurants.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching restaurants {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of restaurantss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Restaurants, field):
                        column = getattr(Restaurants, field)
            ...
            stmt = select(Restaurants, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Restaurants, field_name):
                        stmt = stmt.order_by(getattr(Restaurants, field_name).desc())
                else:
                    if hasattr(Restaurants, sort):
                        stmt = stmt.order_by(getattr(Restaurants, sort))
            else:
                stmt = stmt.order_by(Restaurants.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Restaurants)
                for condition in conditions:
            ...
            logger.error(f"Error fetching restaurants list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Restaurants]:
        """Update restaurants"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Restaurants {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated restaurants {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating restaurants {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Restaurants {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted restaurants {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting restaurants {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Restaurants]:
        """Get restaurants by any field"""
        try:
            if not hasattr(Restaurants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Restaurants")
            column = getattr(Restaurants, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Restaurants).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching restaurants by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Restaurants]:
        """Get list of restaurantss filtered by field"""
        try:
            if not hasattr(Restaurants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Restaurants")
            column = getattr(Restaurants, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Restaurants.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching restaurantss by {field_name}: {str(e)}")
            raise