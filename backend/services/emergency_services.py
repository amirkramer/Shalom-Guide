...
# ------------------ Service Layer ------------------
class Emergency_servicesService:
    """Service layer for Emergency_services operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Emergency_services]:
        """Create a new emergency_services"""
        try:
            obj = Emergency_services(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created emergency_services with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating emergency_services: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Emergency_services]:
        """Get emergency_services by ID"""
        try:
            query = select(Emergency_services).where(Emergency_services.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching emergency_services {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of emergency_servicess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Emergency_services, field):
                        column = getattr(Emergency_services, field)
            ...
            stmt = select(Emergency_services, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Emergency_services, field_name):
                        stmt = stmt.order_by(getattr(Emergency_services, field_name).desc())
                else:
                    if hasattr(Emergency_services, sort):
                        stmt = stmt.order_by(getattr(Emergency_services, sort))
            else:
                stmt = stmt.order_by(Emergency_services.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Emergency_services)
                for condition in conditions:
            ...
            logger.error(f"Error fetching emergency_services list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Emergency_services]:
        """Update emergency_services"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Emergency_services {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated emergency_services {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating emergency_services {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete emergency_services"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Emergency_services {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted emergency_services {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting emergency_services {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Emergency_services]:
        """Get emergency_services by any field"""
        try:
            if not hasattr(Emergency_services, field_name):
                raise ValueError(f"Field {field_name} does not exist on Emergency_services")
            column = getattr(Emergency_services, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Emergency_services).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching emergency_services by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Emergency_services]:
        """Get list of emergency_servicess filtered by field"""
        try:
            if not hasattr(Emergency_services, field_name):
                raise ValueError(f"Field {field_name} does not exist on Emergency_services")
            column = getattr(Emergency_services, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Emergency_services.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching emergency_servicess by {field_name}: {str(e)}")
            raise