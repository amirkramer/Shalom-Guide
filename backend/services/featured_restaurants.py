...
# ------------------ Service Layer ------------------
class Featured_restaurantsService:
    """Service layer for Featured_restaurants operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Featured_restaurants]:
        """Create a new featured_restaurants"""
        try:
            obj = Featured_restaurants(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created featured_restaurants with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating featured_restaurants: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Featured_restaurants]:
        """Get featured_restaurants by ID"""
        try:
            query = select(Featured_restaurants).where(Featured_restaurants.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching featured_restaurants {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of featured_restaurantss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Featured_restaurants, field):
                        column = getattr(Featured_restaurants, field)
            ...
            stmt = select(Featured_restaurants, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Featured_restaurants, field_name):
                        stmt = stmt.order_by(getattr(Featured_restaurants, field_name).desc())
                else:
                    if hasattr(Featured_restaurants, sort):
                        stmt = stmt.order_by(getattr(Featured_restaurants, sort))
            else:
                stmt = stmt.order_by(Featured_restaurants.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Featured_restaurants)
                for condition in conditions:
            ...
            logger.error(f"Error fetching featured_restaurants list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Featured_restaurants]:
        """Update featured_restaurants"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Featured_restaurants {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated featured_restaurants {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating featured_restaurants {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete featured_restaurants"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Featured_restaurants {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted featured_restaurants {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting featured_restaurants {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Featured_restaurants]:
        """Get featured_restaurants by any field"""
        try:
            if not hasattr(Featured_restaurants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Featured_restaurants")
            column = getattr(Featured_restaurants, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Featured_restaurants).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching featured_restaurants by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Featured_restaurants]:
        """Get list of featured_restaurantss filtered by field"""
        try:
            if not hasattr(Featured_restaurants, field_name):
                raise ValueError(f"Field {field_name} does not exist on Featured_restaurants")
            column = getattr(Featured_restaurants, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Featured_restaurants.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching featured_restaurantss by {field_name}: {str(e)}")
            raise