...
# ------------------ Service Layer ------------------
class Transport_routesService:
    """Service layer for Transport_routes operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Transport_routes]:
        """Create a new transport_routes"""
        try:
            obj = Transport_routes(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created transport_routes with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating transport_routes: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Transport_routes]:
        """Get transport_routes by ID"""
        try:
            query = select(Transport_routes).where(Transport_routes.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching transport_routes {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of transport_routess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Transport_routes, field):
                        column = getattr(Transport_routes, field)
            ...
            stmt = select(Transport_routes, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Transport_routes, field_name):
                        stmt = stmt.order_by(getattr(Transport_routes, field_name).desc())
                else:
                    if hasattr(Transport_routes, sort):
                        stmt = stmt.order_by(getattr(Transport_routes, sort))
            else:
                stmt = stmt.order_by(Transport_routes.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Transport_routes)
                for condition in conditions:
            ...
            logger.error(f"Error fetching transport_routes list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Transport_routes]:
        """Update transport_routes"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Transport_routes {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated transport_routes {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating transport_routes {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete transport_routes"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Transport_routes {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted transport_routes {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting transport_routes {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Transport_routes]:
        """Get transport_routes by any field"""
        try:
            if not hasattr(Transport_routes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Transport_routes")
            column = getattr(Transport_routes, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Transport_routes).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching transport_routes by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Transport_routes]:
        """Get list of transport_routess filtered by field"""
        try:
            if not hasattr(Transport_routes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Transport_routes")
            column = getattr(Transport_routes, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Transport_routes.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching transport_routess by {field_name}: {str(e)}")
            raise