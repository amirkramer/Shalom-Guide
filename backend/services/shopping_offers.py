...
# ------------------ Service Layer ------------------
class Shopping_offersService:
    """Service layer for Shopping_offers operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Shopping_offers]:
        """Create a new shopping_offers"""
        try:
            obj = Shopping_offers(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created shopping_offers with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating shopping_offers: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Shopping_offers]:
        """Get shopping_offers by ID"""
        try:
            query = select(Shopping_offers).where(Shopping_offers.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shopping_offers {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of shopping_offerss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Shopping_offers, field):
                        column = getattr(Shopping_offers, field)
            ...
            stmt = select(Shopping_offers, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Shopping_offers, field_name):
                        stmt = stmt.order_by(getattr(Shopping_offers, field_name).desc())
                else:
                    if hasattr(Shopping_offers, sort):
                        stmt = stmt.order_by(getattr(Shopping_offers, sort))
            else:
                stmt = stmt.order_by(Shopping_offers.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Shopping_offers)
                for condition in conditions:
            ...
            logger.error(f"Error fetching shopping_offers list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Shopping_offers]:
        """Update shopping_offers"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shopping_offers {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated shopping_offers {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating shopping_offers {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete shopping_offers"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Shopping_offers {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted shopping_offers {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting shopping_offers {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Shopping_offers]:
        """Get shopping_offers by any field"""
        try:
            if not hasattr(Shopping_offers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shopping_offers")
            column = getattr(Shopping_offers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shopping_offers).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching shopping_offers by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Shopping_offers]:
        """Get list of shopping_offerss filtered by field"""
        try:
            if not hasattr(Shopping_offers, field_name):
                raise ValueError(f"Field {field_name} does not exist on Shopping_offers")
            column = getattr(Shopping_offers, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Shopping_offers)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Shopping_offers.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching shopping_offerss by {field_name}: {str(e)}")
            raise