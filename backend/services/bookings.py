...
class BookingsService:
    """Service layer for Bookings operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bookings]:
        ...
        try:
            ...
            logger.info(f"Created bookings with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating bookings: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for bookings {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Bookings]:
        """Get bookings by ID (user can only see their own records)"""
        try:
            query = select(Bookings).where(Bookings.id == obj_id)
            if user_id:
                query = query.where(Bookings.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bookings {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of bookingss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Bookings.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Bookings, field):
                        column = getattr(Bookings, field)
            ...
            stmt = select(Bookings, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Bookings, field_name):
                        stmt = stmt.order_by(getattr(Bookings, field_name).desc())
                else:
                    if hasattr(Bookings, sort):
                        stmt = stmt.order_by(getattr(Bookings, sort))
            else:
                stmt = stmt.order_by(Bookings.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Bookings)
                for condition in conditions:
            ...
            logger.error(f"Error fetching bookings list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Bookings]:
        """Update bookings (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bookings {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated bookings {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating bookings {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete bookings (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Bookings {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted bookings {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting bookings {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Bookings]:
        """Get bookings by any field"""
        try:
            if not hasattr(Bookings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bookings")
            column = getattr(Bookings, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Bookings).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching bookings by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Bookings]:
        """Get list of bookingss filtered by field"""
        try:
            if not hasattr(Bookings, field_name):
                raise ValueError(f"Field {field_name} does not exist on Bookings")
            column = getattr(Bookings, field_name)
            ...
            logger.error(f"Error fetching bookingss by {field_name}: {str(e)}")
            raise