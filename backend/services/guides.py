...
# ------------------ Service Layer ------------------
class GuidesService:
    """Service layer for Guides operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Guides]:
        """Create a new guides"""
        try:
            if user_id:
            ...
            logger.info(f"Created guides with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating guides: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for guides {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Guides]:
        """Get guides by ID (user can only see their own records)"""
        try:
            query = select(Guides).where(Guides.id == obj_id)
            if user_id:
                query = query.where(Guides.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching guides {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of guidess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Guides.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Guides, field):
                        column = getattr(Guides, field)
            ...
            stmt = select(Guides, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Guides, field_name):
                        stmt = stmt.order_by(getattr(Guides, field_name).desc())
                else:
                    if hasattr(Guides, sort):
                        stmt = stmt.order_by(getattr(Guides, sort))
            else:
                stmt = stmt.order_by(Guides.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Guides)
                for condition in conditions:
            ...
            logger.error(f"Error fetching guides list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Guides]:
        """Update guides (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Guides {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated guides {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating guides {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete guides (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Guides {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted guides {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting guides {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Guides]:
        """Get guides by any field"""
        try:
            if not hasattr(Guides, field_name):
                raise ValueError(f"Field {field_name} does not exist on Guides")
            column = getattr(Guides, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Guides).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching guides by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Guides]:
        """Get list of guidess filtered by field"""
        try:
            if not hasattr(Guides, field_name):
                raise ValueError(f"Field {field_name} does not exist on Guides")
            column = getattr(Guides, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Guides.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching guidess by {field_name}: {str(e)}")
            raise